...
def _sad_cmd_parser(in_cfg: Config):
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="Solana Account Decoder"
    )

    parser.add_argument("-V", "--version", help="Prints version information", action="store_true")

    subparsers = parser.add_subparsers(help="Data from", dest="action")
    ...
    parent_parser.add_argument(
        "-d", "--declfile", help="YAML data declaration file", required=False, dest="decl", action="store"
    )
    parent_parser.add_argument(
        "-u",
        "--url",
    ...
    key_group.add_argument("-k", "--keyfile", help="Account or program keyfile", dest="keyfile", action="store")
    key_group.add_argument("-p", "--pubkey", help="Account or program publickey string", dest="pkstr", action="store")
    ...
    parser_account = subparsers.add_parser("account", parents=[parent_parser], help="Deserialize a ingle account")
    parser_program = subparsers.add_parser(
        "program", parents=[parent_parser], help="Deserialize all program owned accounts"
    )
    return parser
...
def sad_command_line() -> dict:
    ...
    if args.output == "json":
        if args.filename is None:
            raise ValueError("Expected '-f' or '-filename' argument when using -o json")
        else:
